...
from lore.api.resource import ResourceView, ResourceAccessPolicy, ImprovedModelConverter, ImprovedBaseForm, ListResponse, ItemResponse, filterable_fields_parser
...
class ShortcutsView(ResourceView):
    ...
    form_class = model_form(
        Shortcut, base_class=ImprovedBaseForm, converter=ImprovedModelConverter())
    ...
    def get(self, id):
        if id == 'post':
            r = ItemResponse(ShortcutsView, [('shortcut', None)], extra_args={
                             'intent': 'post'})
    ...
    if request.method == 'GET':
        ...
        if not current_app.debug:
            for block in hook_blocks:
                ...
                logger.warning('git_webhook: Incorrect IP: %s not in %s' % (
        ...
        try:
            ...
            tar = subprocess.check_output(
                ('tar', 'xz', '--strip=1'), stdin=curl.stdout, cwd=cwd)
        except subprocess.CalledProcessError as cpe:
            logger.warning('Error fetching repo for {data}, got {out}'.format(
                data=repo_meta, out=cpe.output))
