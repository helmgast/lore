...
class Group(db.Model):
    ...
    def addMasters(self, masters):
        if not masters or not isinstance(masters[0], User):
            raise TypeError("Need a list of Users, got %s of type %s" % (masters, type(masters)))
        to_add = [u for u in masters if u not in self.masters()]
        for u in to_add:
            GroupMaster.create(group=self, master=u)
        return to_add

    def removeMasters(self, masters):
        if not masters or not isinstance(masters[0], User):
            raise TypeError("Need a list of Users, got %s of type %s" % (masters, type(masters)))
        master_instances = GroupMaster.select().where(group=self)
        to_remove = [u for u in master_instances if u.master not in self.masters()]
        to_remove_users = [u.master for u in to_remove]
        for u in to_remove:
            u.delete_instance()
        return to_remove_users

    def players(self):
        return User.select().join(GroupPlayer).where(group=self)
    
    def addPlayers(self, players):
        if not players or not isinstance(players[0], User):
            raise TypeError("Need a list of Users, got %s of type %s" % (players, type(players)))
        to_add = [u for u in players if u not in self.players()]
        for u in to_add:
            GroupPlayer.create(group=self, player=u)
        return to_add

    def removePlayers(self, players):
        if not players or not isinstance(players[0], User):
            raise TypeError("Need a list of Users, got %s of type %s" % (players, type(players)))
        player_instances = GroupPlayer.select().where(group=self).execute()
        to_remove = [u for u in player_instances if u.player in players]
        print to_remove
        print [u for u in player_instances]
        to_remove_users = [u.player for u in to_remove]
        for u in to_remove:
            u.delete_instance()
        return to_remove_users
                
# An autogenerated form class for doing validation and POST / form processing
GroupForm = model_form(Group, exclude=['slug'])
